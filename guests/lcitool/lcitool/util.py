...
def findall(key, d, cb=None):
    """
    Looks up @key in a nested dictionary and returns its value(s).
    There may be multiple occurrences of @key in @d, but not within the
    same subtree. See the example below.
        Example:
            ...
            --> yields <value1>, <value2>, <value3>

            # This won't work as expected
            {'foo':
                {'bar1': {'key': {'foobar': {'key': <value1>}}}},
                {'bar2': {'key': <value2>}},
                {'bar3': {'key': <value3>}},
            }
            --> yields {'foobar': {'key': <value1>}}, <value2>, <value3>

    :param key: key to look for in the dictionary
    :param d: dictionary to traverse
    :param cb: callback to call on @d[*][key] if @key matches
    :returns: generator object or None.
    """
